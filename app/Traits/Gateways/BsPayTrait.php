...
trait BsPayTrait
{
    ...
    {
        \Log::info('BsPayTrait::generateCredentials iniciado');
        
        $setting = Gateway::first();
        if(!empty($setting)) {
            \Log::info('BsPayTrait: Gateway encontrado', [
                'uri' => $setting->bspay_uri,
                'client_id' => $setting->bspay_cliente_id,
                'client_secret' => $setting->bspay_cliente_secret
            ]);
            ...
            \Log::info('BsPayTrait: Credenciais configuradas', [
                'uri' => self::$uri,
        ...
        \Log::error('BsPayTrait: Gateway não encontrado');
        return false;
    }
    ...
    {
        \Log::info('BsPayTrait::authentication iniciado');
        
        $client_id      = self::$clienteId;
        $client_secret  = self::$clienteSecret;
        $credentials = base64_encode($client_id . ":" . $client_secret);

        \Log::info('BsPayTrait: Tentando autenticar', [
            'url' => self::$uri.'oauth/token',
            'client_id' => $client_id,
        ...
        \Log::info('BsPayTrait: Resposta da autenticação', [
        ...
        if ($response->successful()) {
            $data = $response->json();
            \Log::info('BsPayTrait: Token obtido com sucesso', [
                'access_token' => $data['access_token'] ?? 'não encontrado'
            ]);
            return $data['access_token'];
        } else {
            self::$statusCode = $response->status();
            self::$errorBody = $response->body();
            \Log::error('BsPayTrait: Falha na autenticação', [
    ...
    {
        \Log::info('BsPayTrait::requestQrcode iniciado');
        ...
        if($access_token = self::generateCredentials()) {
            ...
            \Log::info('BsPayTrait: Dados da requisição', $request->all());

            $validator = Validator::make($request->all(), $rules);
            if ($validator->fails()) {
                \Log::error('BsPayTrait: Validação falhou', $validator->errors()->toArray());
                return response()->json($validator->errors(), 400);
            }

            \Log::info('BsPayTrait: Validação passou');
            ...
            \Log::info('BsPayTrait: Parâmetros preparados', $parameters);
            ...
            \Log::info('BsPayTrait: Resposta da API', [
                'status' => $response->status(),
                'body' => $response->body()
            ]);
            ...
            if ($response->successful()) {
                ...
                self::$statusCode = $response->status();
                self::$errorBody = $response->body();
                \Log::error('BsPayTrait: Erro na API', [
                    'status' => self::$statusCode,
                    'body' => self::$errorBody
                ]);
                return false;
            ...
            \Log::error('BsPayTrait: Falha ao obter token de autenticação');
            return false;
        }
    }
